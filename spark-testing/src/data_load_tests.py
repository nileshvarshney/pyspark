...
from file_processor import find_the_late_flights, enrich_with_flight_desc, convert_column_case
...
def test_convert_case(flight_desc_df):
    new_flight_desc_df = convert_column_case(flight_desc_df)
    new_flight_desc_df.show(truncate=False)
    assert new_flight_desc_df.count() == 2
