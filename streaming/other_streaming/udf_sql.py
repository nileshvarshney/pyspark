...
def main():
    ...
    # register UDF
    sparkSession.udf.register("add_rating_udf",calcualte_rating,StringType())
    
    sm_sales_df.createOrReplaceTempView('Market')
    
    output_df = sparkSession.sql("""
            Select 
                Branch,City,`Product line`, 
                Rating , add_rating_udf(Rating) as comments
            FROM Market """
            )
